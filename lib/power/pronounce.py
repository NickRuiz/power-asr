...
class PronouncerLex(object):
	...
	def pronounce(self, words):
		...
		prons = [self.lexicon[w] if w in self.lexicon else self.pyphen_pronounce(w) for w in wordsLower]
	...
	def pyphen_pronounce(self, word):
		...
		for syl in syllables:
			...
			sylpron = []
			while m < n:
				j, p = [(n-i, self.lexicon[syl[m:n-i]]) for i in range(n) if syl[m:n-i] in self.lexicon][0]
				sylpron.append(p)
				m = j
