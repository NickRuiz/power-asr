...
class ExpandedAlignment:
	...
	def __init__(self, s1, s2, align, s1_map=None, s2_map=None, lowercase=False):	
		if not (len(s1) == len(s2) == len(align)):
			raise Exception("Length mismatch: align:{0:d}, s1:{1:d}, s2:{2:d}".format(len(align), len(s1), len(s2)))
		if len(align) == 0:
	...
	def __str__(self):
		widths = [max(len(self.s1[i]), len(self.s2[i])) for i in range(len(self.s1))]
	...
	def subsequence(self, i, j, preserve_index=False):
		...
		s1_map = [self.s1_map[k] - scale for k in range(len(self.s1_map)) if i <= self.s1_map[k] < j]
		s2_map = [self.s2_map[k] - scale for k in range(len(self.s2_map)) if i <= self.s2_map[k] < j]
	...
	def append_alignment(self, expanded_alignment):
		...
		if self.s1_map and expanded_alignment.s1_map:
			self.s1_map += [align_pos + map_offset for align_pos in expanded_alignment.s1_map]
		if self.s2_map and expanded_alignment.s2_map:
			self.s2_map += [align_pos + map_offset for align_pos in expanded_alignment.s2_map]
	...
	def error_rate(self, cluster_on_ref=False):
		...
		'''			
		score_components = {AlignLabels.correct:0, AlignLabels.substitution:0, AlignLabels.deletion:0, AlignLabels.insertion:0, 'L':0}
		...
		if not self.s1:
			...
			error_rate = (score_components[AlignLabels.substitution] + score_components[AlignLabels.deletion] + score_components[AlignLabels.insertion]) / score_components['L']
...
class Levenshtein:
	...
	uniformWeights    = {AlignLabels.correct: 0, AlignLabels.substitution: 1, AlignLabels.deletion: 1, AlignLabels.insertion: 1}
	wordAlignWeights  = {AlignLabels.correct: 0, AlignLabels.substitution: 4, AlignLabels.deletion: 3, AlignLabels.insertion: 3}
...
# 					delPenalty += dist_penalty * weights[AlignLabels.deletion] * (1 - 1 / (distPenaltyHyp + 1))
				...
				if char1 == char2:
					opts.append(lev.backMatrix.getWeight(index2, index1) + weights[AlignLabels.correct]) # C
...
# 						if char1_set == char2_set:
						if set.intersection(char1_sets, char2_sets):
							opts.append(lev.backMatrix.getWeight(index2, index1) + weights[AlignLabels.substitution]) # S
					else:
						opts.append(lev.backMatrix.getWeight(index2, index1) + weights[AlignLabels.substitution])     # S
				
				# Get the locally minimum score and edit operation
				minDist = min(opts)
				minIndices = [i for i in reversed(xrange(len(opts))) if opts[i] == minDist] 
...
# 			print self.s1[j-1], self.s2[i-1]
	...
	def expandAlignCompact(self, minPos=None, maxPos=None):
		...
		s1_align, s2_align, align = [list(a) for a in zip(*(G[u][v]['labels'] for (u,v) in zip(path[0:],path[1:])))]
...
class BackTrackMatrix:
	def __init__(self, reflen, hyplen, weights=Levenshtein.uniformWeights):
		...
		for i in range(1, self.row_count):
			self.addBackTrack(i, 0, AlignLabels.insertion, i*weights[AlignLabels.insertion])
			
		# Initialize columns.
		for j in range(1, self.col_count):
			self.addBackTrack(0, j, AlignLabels.deletion, j*weights[AlignLabels.deletion])
...
class BackTrackSlot:
	...
	def addOptions(self, alignLabels):
		self.backTrackOptions.extend([x for x in alignLabels if x not in self.backTrackOptions])
